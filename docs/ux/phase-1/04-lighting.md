...
- `02-design-system.md` — デザインシステム（モーショントークン）
- `03-assets.md` — レイヤースタック・合成方式

**参照ドキュメント（World Bible）:**
- `../world-bible/visual-foundation.md` — 視覚言語の基礎
- `../world-bible/experience-principles.md` — 体験原則（世界と同調する）
...
このアプリにおけるライティングは、ダークモード／ライトモードの切替ではない。

それは、キッチン世界の「空気」そのものを作る仕組みである。

UIの色を変更する機能ではなく、空間全体の感情温度を決定するレイヤーとして存在する。

- ダークモード/ライトモードの切替は**禁止**
- ボタンでのテーマ変更は**禁止**
- ライティングは「自然現象」として扱う

### 感情リアリティ優先（Emotional Realism）

現実 5 : ファンタジー 5 の原則に基づき、物理的な色温度の正確さよりも「印象」を優先する。

- 物理的な色温度の正確さより「印象」を優先
- 「朝っぽい」「夜っぽい」と感じられればよい
- 現実再現ではなく、感情演出
- 形状はファンタジー寄りでよいが、光の挙動は生活感に忠実であること

### 現地時間を使用する

ライティングは必ずユーザーの「現地時間（ローカルタイム）」に基づいて決定する。

- サーバー時間は使用しない
- タイムゾーンを尊重する
- 旅行・引越し時も自動追従する

このアプリは、ユーザーの生活と同じ時間を生きる。
...
**注:** Soft Light / Overlay / Multiply 効果はアセット制作時に焼き込む。ランタイムでは Normal + opacity のみ使用（03-assets.md §実装互換性 参照）。
...
- **motion tokens 参照**: クロスフェードに `duration.transition.overlay` (300ms)、`easing.easeOut` を使用（02-design-system.md 参照）
